...
lowerAndGreater n list = show n ++ " is greater than " ++ show g ++ " elements and lower than " ++ show s ++ " elements"
    where
        countCases :: (Int -> Int -> Bool) -> Int -> [Int] -> Int
        countCases func num lst = go func num 0 lst
            where
                go :: (Int -> Int -> Bool) -> Int -> Int -> [Int] -> Int
                go f x result l
                  | null l = result
                  | f x (head l) = go f x (result + 1) (tail l)
                  | otherwise = go f x result (tail l)
        g = countCases (>) n list
        s = countCases (<) n list
